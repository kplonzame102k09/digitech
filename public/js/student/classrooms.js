(() => {
    ...
    wrap.innerHTML = classrooms.map((c) => `
      <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
        ...
        <div class="mt-3 flex flex-wrap gap-2">
          <a href="/student/classrooms/${esc(c.id)}" class="rounded bg-slate-900 px-4 py-2 text-xs font-semibold text-white dark:bg-slate-100 dark:text-slate-900">Open classroom</a>
          <button type="button" data-view-activities="${esc(c.id)}" class="rounded border px-4 py-2 text-xs font-semibold">Quick submit</button>
        </div>
  ...
  async function toggleActivities(classroomId, btn) {
    ...
    box.innerHTML = `<p class="text-sm text-slate-500 dark:text-slate-400">Loading…</p>`;
    try {
      ...
      const groupsHtml = groups.length
        ? groups.map((g) => `
          <div>
            ...
            return `<div class="rounded bg-slate-50 p-3 dark:bg-slate-800">
        ...
        : `<p class="text-sm text-slate-500 dark:text-slate-400">No activities yet.</p>`;
      ...
      box.innerHTML = `<p class="text-sm text-rose-600 dark:text-rose-400">${esc(error.message || "Failed to load activities.")}</p>`;
    }
  }
  ...
  // Live updates: reload the classroom list when idle.
