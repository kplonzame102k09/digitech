(() => {
  ...
  async function renderDashboard() {
    ...
    try {
      ...
      if (enrollBox) {
        enrollBox.innerHTML = enr
          ...
          : `<p class="text-sm text-slate-500 dark:text-slate-400">You have not started enrollment yet.</p>`;
      }
    } catch (error) {
    ...
    if (activity) {
      ...
      activity.innerHTML = notes.length
        ? notes
            .map(
              (n) => `<div class="rounded bg-slate-50 p-3 dark:bg-slate-800">
                <b class="text-sm">${esc(n.title)}</b>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">${esc(n.message || "")}</p>
              </div>`,
            )
            .join("")
        : `<p class="text-sm text-slate-500 dark:text-slate-400">No recent activity yet.</p>`;
    }
    ...
    if (list) {
      list.innerHTML = announce.length
        ? announce
            .map(
              (a) => `<div class="rounded bg-slate-50 p-3 dark:bg-slate-800">
                <b class="text-sm">${esc(a.title)}</b>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">${esc(a.message || "")}</p>
              </div>`,
            )
            .join("")
  ...
  async function renderEnrollment() {
    ...
    try {
      ...
      if (locked) {
        ...
        if (status) {
          status.innerHTML +=
            '<p class="mt-2 text-xs font-semibold text-amber-600 dark:text-amber-400">This enrollment is locked — contact the registrar to make changes.</p>';
        }
      }
  ...
  async function renderGrades() {
    try {
      ...
      const body = $("#rows");
      if (body) {
        body.innerHTML = `<tr><td colspan="7" class="p-8 text-center text-slate-500 dark:text-slate-400">Error loading grades.</td></tr>`;
      }
    }
  }
  ...
  async function renderDocuments() {
    try {
      ...
      const body = $("#rows");
      if (body) {
        body.innerHTML = `<tr><td colspan="4" class="p-8 text-center text-slate-500 dark:text-slate-400">Error loading document requests.</td></tr>`;
      }
    }
  ...
  function openRequirementSubmit(record) {
    ...
    panel.innerHTML = `
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-400">Upload requirement</p>
          <h2 class="mt-1 text-lg font-extrabold">${esc(record.name)}</h2>
          <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">${esc(record.status)}${record.fileUrl ? ` · attached: ${esc(fileLabel(record.fileUrl))}` : ""}</p>
        </div>
        <button type="button" data-close-req class="rounded p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Close"><i data-lucide="x" class="h-5 w-5"></i></button>
      </div>
      <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Choose the document below and upload it. Uploading marks this requirement as submitted.</p>
      <label class="mt-4 block text-sm font-semibold">
        Document file
        <input id="reqFile" type="file" accept=".pdf,image/jpeg,image/png,image/webp" class="input mt-1.5 w-full rounded border px-3 py-2.5">
      </label>
      <p class="mt-2 text-xs text-slate-400">PDF or image up to 5 MB.</p>
      <div class="mt-6 flex justify-end gap-3">
        <button type="button" data-cancel-req class="rounded border border-slate-200 px-4 py-2.5 text-sm font-semibold dark:border-slate-700">Cancel</button>
        <button id="reqSubmit" class="inline-flex items-center gap-2 rounded bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700"><i data-lucide="upload-cloud" class="h-4 w-4"></i>Upload</button>
      </div>`;
  ...
  function renderRequirements() {
    ...
    body.innerHTML = rows.length
      ? rows
              ...
              <td class="p-4 text-right">
                ${
                  r.status === "Pending" || r.status === "Rejected"
                    ? `<button data-upload-req="${esc(r.id)}" class="inline-flex items-center gap-1.5 rounded bg-emerald-600 px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-emerald-700"><i data-lucide="upload-cloud" class="h-4 w-4"></i>${r.status === "Rejected" ? "Re-upload" : "Upload"}</button>`
                    : "—"
                }
              </td>
            </tr>`,
          ...
          .join("")
      : `<tr><td colspan="4" class="p-8 text-center text-slate-500 dark:text-slate-400">No requirements assigned yet.</td></tr>`;
  ...
  // Live updates: re-run the current page render when data changes elsewhere.
