...
<body class="min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100">
            ...
            <div class="mx-auto max-w-7xl">
                    ...
                    <div class="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        <div>
                            ...
                            <p class="mt-2 max-w-2xl text-white">
                                Create a classroom and invite students with a link or a 9-digit code.
                            </p>
                        </div>
                ...
                <section class="mb-6 grid gap-4 sm:grid-cols-3">
                    <div class="teacher-stat card p-5">
                        <p class="text-sm font-medium text-slate-500">Active classrooms</p>
                        <p id="classroomTotal" class="mt-2 text-3xl font-extrabold">0</p>
                        <p class="mt-1 text-xs text-slate-400">Open for joining</p>
                    </div>
                    <div class="teacher-stat card p-5">
                        <p class="text-sm font-medium text-slate-500">Students joined</p>
                        <p id="classroomStudents" class="mt-2 text-3xl font-extrabold text-emerald-600">0</p>
                        <p class="mt-1 text-xs text-slate-400">Across active classrooms</p>
                    </div>
                    <div class="teacher-stat card p-5">
                        <p class="text-sm font-medium text-slate-500">Archived</p>
                        <p id="classroomArchived" class="mt-2 text-3xl font-extrabold text-slate-500">0</p>
                ...
                <section class="card p-5 sm:p-6">
                    ...
                    <div id="classroomEmpty" class="hidden py-10 text-center text-sm text-slate-500">
    ...
    <dialog id="classroomDialog"
        ...
        <form id="classroomForm" method="dialog" class="card p-6">
            ...
            <p class="mt-1 text-sm text-slate-500">Students join with the link or 9-digit code we generate.</p>
            <div class="mt-4 grid gap-4">
                <label class="text-sm font-semibold">Classroom name
                    <input id="classroomName" maxlength="50" required
                        class="input mt-1.5 w-full rounded-xl border px-3 py-2.5" placeholder="e.g. Grade 11 - STEM A" />
                </label>
                <label class="text-sm font-semibold">Subject <span class="font-normal text-rose-500">* fixed — cannot be changed later</span>
                    <input id="classroomSubject" maxlength="120" required
                        class="input mt-1.5 w-full rounded-xl border px-3 py-2.5" placeholder="e.g. Mathematics" />
                </label>
                <label class="text-sm font-semibold">Description <span class="font-normal text-slate-400">(optional)</span>
                    <textarea id="classroomDescription" rows="3" maxlength="2000"
                        class="input mt-1.5 w-full rounded-xl border px-3 py-2.5" placeholder="Schedule, room, notes"></textarea>
                </label>
            </div>
            <div class="mt-6 flex justify-end gap-3">
                <button type="button" data-close-classroom
                    class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold dark:border-slate-700">Cancel</button>
                <button class="rounded-xl bg-emerald-600 px-5 py-2.5 font-semibold text-white hover:bg-emerald-700">Create</button>
            </div>
        </form>
    </dialog>
