<!doctype html>
<html>

<head>
    @include('partials.meta', ['pageTitle' => 'Announcements | Digitech College'])
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = { darkMode: "class" };
    </script>
    <script src="https://unpkg.com/lucide@latest"></script>
    <link rel="stylesheet" href="{{ asset('css/custom.css') }}" />
</head>

<body data-role="student" data-feature="announcements" class="student-portal min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100">
    @include('student.components.sidebar')
    <div class="lg:pl-64">
        @include('student.components.header', ['title' => 'Announcements', 'subtitle' => 'Campus life'])
        <main class="p-4 sm:p-6 lg:p-8">
            <div class="mx-auto max-w-3xl">
                <section class="mb-6">
                    <p class="text-xs font-bold uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-300">College
                        communication</p>
                    <h2 class="mt-1 text-2xl font-extrabold tracking-tight">Announcements</h2>
                    <p class="mt-1 text-sm text-slate-500">Stay up to date with college and class notices.
                    </p>
                </section>
                <section>
                    <div class="mb-3 flex items-center gap-2 px-1">
                        <i data-lucide="newspaper" class="h-4 w-4 text-slate-400"></i>
                        <h3 class="text-sm font-bold text-slate-500 dark:text-slate-400">Latest updates</h3>
                    </div>
                    <div id="rows" class="space-y-4"></div>
                </section>
            </div>
        </main>
    </div>
    <div id="modalRoot"></div>
@include('partials.portal-scripts', ['portalPage' => 'workflows.js'])
</body>

</html>