<aside id="side"
    ...
    <div class="flex h-full flex-col">
        <div class="flex h-20 items-center gap-3 border-b border-slate-200 px-5 dark:border-slate-800">
            <img src="{{ asset('images/16432.png') }}" class="h-10 w-10 rounded object-cover"
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto p-3">
            <a href="{{ route('parent.dashboard') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
            ...
            <a href="{{ route('parent.children') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
                {{ request()->routeIs('parent.children')
                ? 'nav-active' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}}">
                <i data-lucide="users-round" class="w-4"></i>
                My Children
            </a>
            <a href="{{ route('parent.attendance') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
                {{ request()->routeIs('parent.attendance')
                ? 'nav-active' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}}">
                <i data-lucide="calendar-check-2" class="w-4"></i>
                Attendance
            </a>
            <a href="{{ route('parent.grades') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
                {{ request()->routeIs('parent.grades')
                ? 'nav-active' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}}">
                <i data-lucide="chart-no-axes-combined" class="w-4"></i>
                Grades
            </a>
            <a href="{{ route('parent.documents') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
                {{ request()->routeIs('parent.documents')
                ? 'nav-active' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}}">
                <i data-lucide="file-text" class="w-4"></i>
                Documents
            </a>
            <a href="{{ route('parent.announcements') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
                {{ request()->routeIs('parent.announcements')
                ? 'nav-active' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}}">
                <i data-lucide="megaphone" class="w-4"></i>
                Announcements
            </a>
            <a href="{{ route('parent.profile') }}" class="flex items-center gap-3 rounded px-3 py-2.5 text-sm font-medium
        ...
        <div class="border-t border-slate-200 p-3 dark:border-slate-800">
            <button type="button" data-logout
                class="flex w-full items-center gap-3 rounded px-3 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50 dark:text-red-400">
