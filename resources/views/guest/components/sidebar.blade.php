<aside id="side" class="sidebar fixed inset-y-0 left-0 z-40 w-64 -translate-x-full lg:translate-x-0 bg-white dark:bg-slate-900 border-r dark:border-slate-800">
    <div class="h-full flex flex-col">
      <div class="h-20 border-b dark:border-slate-800 flex items-center gap-3 px-5">
        ...
        <div>
          <b>DIGITECH</b><small class="block text-[10px] tracking-widest text-slate-400">GUEST PORTAL</small>
        </div>
      </div>
      <nav class="p-3 space-y-1 flex-1">
        <a href="{{ route('guest.announcements') }}"
          class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium
          {{ request()->routeIs('guest.announcements') ? 'nav-active' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}}">
          <i data-lucide="megaphone" class="w-4"></i>
            Announcements
        </a>
        <a href="{{ route('guest.documents') }}"
          class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium
          {{ request()->routeIs('guest.documents') ? 'nav-active' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}}">
        ...
        <a href="{{ route('guest.profile') }}"
          class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium
          {{ request()->routeIs('guest.profile') ? 'nav-active' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}}">
